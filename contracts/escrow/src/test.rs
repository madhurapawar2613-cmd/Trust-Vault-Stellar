...
fn setup(env: &Env) -> (
    ...
    (admin, client_addr, freelancer_addr, token_id, token, escrow_client)
}
...
fn test_create_escrow() {
    ...
    let escrow_id = escrow_client.create_escrow(
        &client_addr, &freelancer_addr, &token_id, &two_milestones(&env),
    );
...
fn test_complete_and_approve_milestone() {
    ...
    assert_eq!(token::Client::new(&env, &token_id).balance(&freelancer_addr), 400);
}
...
fn test_full_completion() {
    ...
    assert_eq!(token::Client::new(&env, &token_id).balance(&freelancer_addr), 1000);
...
fn test_raise_dispute() {
    ...
    let escrow_id = escrow_client.create_escrow(
        &client_addr, &freelancer_addr, &token_id, &milestones,
    );
...
fn test_cancel_escrow() {
    ...
    assert_eq!(token::Client::new(&env, &token_id).balance(&client_addr), 800); // refunded
...
fn test_multiple_escrows() {
    ...
    let m1 = Vec::from_array(&env, [Milestone {
        id: 0, description: String::from_str(&env, "Task A"),
        amount: 1000, completed: false, approved: false,
    }]);
    let m2 = Vec::from_array(&env, [Milestone {
        id: 0, description: String::from_str(&env, "Task B"),
        amount: 2000, completed: false, approved: false,
    }]);
